...
public class Snippets {
    ...
    void selectors_examples() {
        ...
        $("").preceding(0).click();
        $("").closest("div").click();
        $("").ancestor("div").click(); // the same as closest
        $("div:last-child").click();
        $("div:visible").click(); // will not work, because not supported in selenium
        ...
        $(byId("mytext")).click(); // use this if id starts with number or special symbol
        $("#mytext").click();
    ...
    void actions_examples() {
        ...
        $("").setValue("text"); // перезаписывает все содержимое
        $("").append("text"); // добавляет текст в конец
        $("").clear(); // sometimes doesn't work correctly
        $("").setValue(""); // clear
        ...
        $("html").sendKeys(Keys.chord(Keys.SPACE));
    ...
    void conditions_examples() {
        ...
        $("").shouldNotBe(checked); // for checkboxes
